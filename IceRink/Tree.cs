...
namespace IceRink
{
    ...
    {
        public Tree(int x, int y, Color color, int size, int step) : base(x, y, color, size, step)
        {
        ...
        new SolidBrush(Color.FromArgb(180, 230, 40))
            ...
            //Alternative implementation with FillPie for leaves:

            /*
            for (int i = 0; i < 3; i++)
            {
                float pieSize = Size * (1.2f - i * 0.2f);
                float pieY = Y - Size * (1.4f - i * 0.2f);

                g.FillPie(leavesBrushes[i], X - pieSize / 2f , pieY, pieSize, pieSize, 235, 130);
            }
            */

            // Dispose of brushes and pens
            trunkPen.Dispose();
            trunkBrush.Dispose();
